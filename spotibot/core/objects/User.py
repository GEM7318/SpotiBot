...
from spotibot.base.auth import \
    OAuth as oauth

from spotibot.core.endpoints import \
    Generic as genhref
...
activity_playlists = \
    config.get_configs(['PLAYLISTS', 'ACTIVITY'])

activity_names_to_desc = \
    {activity_playlists.get('MUSIC').get('NAME'):
        activity_playlists.get('MUSIC').get('DESCRIPTION'),
     activity_playlists.get('PODCASTS').get('NAME'):
         activity_playlists.get('PODCASTS').get('DESCRIPTION')}
...
def get_user_playlists(href, headers):
    ...
    while result.get('next'):
        result = \
            requests.get(result.get('next'), headers=headers).json()
        all_playlists.append(result['items'])

    all_playlists = \
        {t.get('name'): t.get('href')
         for batch in all_playlists for t in batch}
...
class UserDBO(genhref.Href):
    ...
    def __init__(self, user_id: str):
        ...
        self.mongo_user = \
            UserDoc.User.objects(pk=user_id).first()

        self.tokens: oauth.Token = \
            jsonpickle.decode(self.mongo_user.obj)

        # Checking for activity playlists and creating if not pre-existing
        activity_names_to_types = \
            {config.get_configs(
                ['PLAYLISTS', 'ACTIVITY', 'MUSIC', 'NAME']):
                    music.Track,
             config.get_configs(
        ...
        names_to_hrefs = \
            {k: v for k, v in
             get_user_playlists(self.playlists, self.headers()).items()
             if activity_names_to_types.get(k)}

        activity_names_to_create = \
            [k for k in activity_names_to_types.keys()
             if not names_to_hrefs.get(k)]

        for name in activity_names_to_create:
            desc = activity_names_to_desc.get(name)
            req_href, req_payload = \
                self.new_playlist(playlist_name=name, playlist_desc=desc)

            request = requests.post(req_href, data=req_payload,
        ...
        activity_playlist_hrefs = \
            {'track':
                 names_to_hrefs.get(activity_playlists
                                    .get('MUSIC')
                                    .get('NAME')),
             'episode':
                 names_to_hrefs.get(activity_playlists
