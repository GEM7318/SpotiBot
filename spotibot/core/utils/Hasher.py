...
def dict_from_df(df: pd.DataFrame, key_col: str, val_col: str) -> dict:
    ...
    """
    to_return = {k: v[val_col] for k, v in
                 df[[key_col, val_col]].set_index(key_col).to_dict('index').items()
...
def iso8601_to_unix(iso_string, includes_tz = True):
    if includes_tz is True:
        return calendar.timegm(dt.datetime.strptime(iso_string, "%Y-%m-%dT%H:%M:%S.%fZ").timetuple())
    else:
        return calendar.timegm(dt.datetime.strptime(iso_string, "%Y-%m-%dT%H:%M:%S.%f").timetuple())
#  -----------------
