...
def get_activity_playlist_for_user():
    ...
    :return:
    """
    dir_user_activity_playlist = os.path.join(os.getcwd(), '_data',
                                              'UserDBO Playlists',
                                              'py_managed_user_playlist_dim')

    path_to_file = [os.path.join(dir_user_activity_playlist, file) for
                    file in os.listdir(dir_user_activity_playlist)
                    if '.csv' in file][0]
